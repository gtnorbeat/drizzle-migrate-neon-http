...
3. Warns (or throws, with `strict`) about any `*.sql` file in the directory that
   is **not** registered in the journal — such files are never applied.
4. For each file not yet applied (by SHA-256):
...
5. Skips files already applied (hash match).
...
With `retries > 0`, a failed pass is retried from scratch with exponential
backoff (`1s, 2s, 4s, …` capped at `10s`). Re-runs re-read the applied hashes,
so completed files stay skipped and a partially-applied file heals via the
idempotent skips above. With `timeoutMs` set, each driver call is raced against
a deadline — a stalled HTTP request rejects instead of hanging.
...
- an orphaned `.sql` file is present and `strict` is set
- a statement fails with a non-idempotent error (see [errors](/api/errors))
- a query exceeds `timeoutMs`
